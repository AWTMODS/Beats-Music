...
|---|---|---|
| `BeatsPlayerCubit` | `blocs/media_player/` | Wraps `BeatsMusicPlayer`; exposes player state |
...
Plugins are WebAssembly modules conforming to WIT-defined interfaces, compiled via the `Beats_plugin_sdk`. The Rust layer (`rust/`) bridges Flutter and the WASM runtime.
...
| Service | Purpose |
|---|---|
| `BeatsMusicPlayer` | Core audio engine wrapping `audio_service` |
...
App start
  └─ GlobalEventsCubit.checkForUpdates()
       └─ getAppUpdates()          (`lib/services/beats_updater_tools.dart`)
            ├─ GitHub Releases API
            ...
            Result map keys:
              ...
              changelogs → String?: text if unread changelog exists
                  ...
                  └─ results == true  &&  autoUpdateNotify setting == true
                        ...
                        → GlobalEventListener: showBeatsDialog with localised strings
                             ├─ "Later" → dismiss
