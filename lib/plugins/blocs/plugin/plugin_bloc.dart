...
import 'package:beats_music/services/db/dao/plugin_storage_dao.dart';
...
class PluginBloc extends Bloc<PluginEvent, PluginState> {
  ...
  Future<void> _onRefreshPlugins(
    ...
    try {
      ...
      // In Safe Mode, getAvailablePlugins() returns [] because PluginInfo
      // is a Rust opaque type that cannot be constructed in Dart.
      // Preserve the existing availablePlugins list so the UI stays intact.
      final effectiveAvailable = _pluginService.isDeactivated && available.isEmpty
          ? state.availablePlugins
          : available;

      emit(state.copyWith(
        availablePlugins: effectiveAvailable,
        loadedPluginIds: loaded.toSet(),
        isLoading: false,
        pluginOperations: const {},
      ));
  ...
  Future<void> _onSystemEvent(
    ...
    e.when(
      ...
      pluginUnloaded: (pluginId) {
        ...
        final manualUnload =
            state.operationFor(pluginId) == PluginOperation.unloading;
