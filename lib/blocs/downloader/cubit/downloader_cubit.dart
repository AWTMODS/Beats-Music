...
import 'package:beats_music/utils/dload.dart';
...
class DownloaderCubit extends Cubit<DownloaderState> {
  ...
  Future<Directory> _getDownloadDirectory() async {
    ...
    final path =
        await BeatsMusicDBService.getSettingStr(GlobalStrConsts.downPathSetting);
    if (path != null) {
  ...
  Future<void> _loadDownloadedSongs() async {
    final list = await BeatsMusicDBService.getDownloadedSongs();
  ...
  void _onDownloadComplete(DownloadTask task) async {
    ...
    await BeatsMusicDBService.putDownloadDB(
  ...
  Future<bool> _isAlreadyDownloaded(MediaItemModel song) async {
    final dbRecord = await BeatsMusicDBService.getDownloadDB(song);
    if (dbRecord != null) {
      ...
      if (await file.exists()) {
        ...
        log("Stale DB record found for ${song.title}. Removing.",
            name: "DownloaderCubit");
        await BeatsMusicDBService.removeDownloadDB(song);
        return false;
      }
    }
  ...
  Future<void> downloadSong(MediaItemModel song,
    ...
    try {
      ...
      if (song.extras!['source'] == 'youtube' ||
        ...
        await BeatsMusicDBService.getSettingStr(GlobalStrConsts.ytDownQuality)
            .then((quality) {
          if (quality == "High") {
        ...
        final quality =
            await BeatsMusicDBService.getSettingStr(GlobalStrConsts.downQuality);
        if (quality == "High") {
          downloadUrl =
