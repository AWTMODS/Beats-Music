...
class ArtistCubit extends Cubit<ArtistState> {
  ...
  Future<void> checkIsSaved() async {
    bool isSaved = await BeatsMusicDBService.isInSavedCollections(artist.sourceId);
  ...
  Future<void> addToSavedCollections() async {
    if (!state.isSavedCollection) {
      await BeatsMusicDBService.putOnlArtistModel(artist);
      SnackbarService.showMessage("Artist added to Library!");
    } else {
      await BeatsMusicDBService.removeFromSavedCollecs(artist.sourceId);
