...
class OnlPlaylistCubit extends Cubit<OnlPlaylistState> {
  ...
  Future<void> checkIsSaved() async {
    bool isSaved =
        await BeatsMusicDBService.isInSavedCollections(playlist.sourceId);
  ...
  Future<void> addToSavedCollections() async {
    if (!state.isSavedCollection) {
      await BeatsMusicDBService.putOnlPlaylistModel(playlist);
      SnackbarService.showMessage("Artist added to Library!");
    } else {
      await BeatsMusicDBService.removeFromSavedCollecs(playlist.sourceId);
