...
import 'package:beats_music/repository/MixedAPI/mixed_api.dart';
...
class LastdotfmCubit extends Cubit<LastdotfmState> {
  ...
  Future<void> songTimeTracker() async {
    ...
    scrobbleSub = playerCubit.progressStreams.listen((event) {
      if (playerCubit.beatsMusicPlayer.audioPlayer.playing &&
          event.currentPlaybackState.processingState == ProcessingState.ready) {
        if (lastPlayed != playerCubit.beatsMusicPlayer.currentMedia ||
            !stopwatch.isRunning) {
          ...
          stopwatch.start();
          lastPlayed = playerCubit.beatsMusicPlayer.currentMedia;
        } else if ((stopwatch.elapsed.inSeconds > 30 ||
                (stopwatch.elapsed.inSeconds /
                        (playerCubit.beatsMusicPlayer.currentMedia.duration ??
                                const Duration(
                                    hours:
                                        1)) // if duration is null, set it to 1 hour to avoid division by zero
                            .inSeconds) >
                    0.5) &&
            playerCubit.beatsMusicPlayer.currentMedia == lastPlayed &&
            playerCubit.beatsMusicPlayer.currentMedia != playedMedia.value) {
          playedMedia.add(playerCubit.beatsMusicPlayer.currentMedia);
          log("Scrobbling: ${playerCubit.beatsMusicPlayer.currentMedia.title}",
              name: "Last.FM");
      ...
      } else if (lastPlayed != playerCubit.beatsMusicPlayer.currentMedia) {
        stopwatch.stop();
        stopwatch.reset();
      } else {
        stopwatch.stop();
  ...
  Future<void> initializeFromDB() async {
    ...
    final username =
        await BeatsMusicDBService.getApiTokenDB(GlobalStrConsts.lFMUsername);
    final apiKey =
        await BeatsMusicDBService.getApiTokenDB(GlobalStrConsts.lFMApiKey);
    final apiSecret =
        await BeatsMusicDBService.getApiTokenDB(GlobalStrConsts.lFMSecret);
    final session =
        await BeatsMusicDBService.getApiTokenDB(GlobalStrConsts.lFMSession);
  ...
  Future<void> fetchSessionkey(
    ...
    try {
      ...
      BeatsMusicDBService.putApiTokenDB(GlobalStrConsts.lFMUsername, name, "0");
      BeatsMusicDBService.putApiTokenDB(GlobalStrConsts.lFMSecret, secret, "0");
      BeatsMusicDBService.putApiTokenDB(GlobalStrConsts.lFMApiKey, apiKey, "0");
      BeatsMusicDBService.putApiTokenDB(GlobalStrConsts.lFMSession, session, "0");
  ...
  Future<void> remove() async {
    ...
    BeatsMusicDBService.putApiTokenDB(GlobalStrConsts.lFMSecret, "", "0");
    BeatsMusicDBService.putApiTokenDB(GlobalStrConsts.lFMApiKey, "", "0");
    BeatsMusicDBService.putApiTokenDB(GlobalStrConsts.lFMSession, "", "0");
    BeatsMusicDBService.putApiTokenDB(GlobalStrConsts.lFMUsername, "", "0");
  }
  ...
  Future<bool> scrobble(MediaItemModel mediaItem) async {
    final shouldScrobble = await BeatsMusicDBService.getSettingBool(
  ...
  Future<List<ScrobbleTrack>> getLFMTrackedCache() async {
    final trackList =
        await BeatsMusicDBService.getAPICache(GlobalStrConsts.lFMTrackedCache);
    await BeatsMusicDBService.putAPICache(GlobalStrConsts.lFMTrackedCache, "null");
