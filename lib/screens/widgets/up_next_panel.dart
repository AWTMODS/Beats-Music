...
import 'package:beats_music/screens/widgets/toogle_btn.dart';
...
class _UpNextPanelState extends State<UpNextPanel> {
  ...
  late final BeatsPlayerCubit _playerCubit;

  @override
  void initState() {
    super.initState();
    _playerCubit = context.read<BeatsPlayerCubit>();
    
    // Subscribe to media item changes for auto-scroll
    _mediaItemSub = _playerCubit.beatsMusicPlayer.mediaItem.listen((value) {
      if (value != null && mounted) {
        // Trigger auto-scroll logic
      }
    });
...
class _PanelContent extends StatelessWidget {
  ...
  final BeatsPlayerCubit playerCubit;
      ...
      child: Row(
        ...
        children: [
          StreamBuilder<List<MediaItem>>(
            stream: playerCubit.beatsMusicPlayer.queue,
...
class _DesktopSongListState extends State<_DesktopSongList> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<MediaItem>>(
      stream: widget.playerCubit.beatsMusicPlayer.queue,
      builder: (context, queueSnapshot) {
        ...
        return StreamBuilder<MediaItem?>(
          stream: widget.playerCubit.beatsMusicPlayer.mediaItem,
          builder: (context, mediaSnapshot) {
            ...
            return ReorderableListView.builder(
              ...
              onReorder: (int oldIndex, int newIndex) {
                widget.playerCubit.beatsMusicPlayer
  ...
  Widget build(BuildContext context) {
    ...
    return Dismissible(
      ...
      onDismissed: (direction) {
        playerCubit.beatsMusicPlayer.removeQueueItemAt(index);
      },
      child: Material(
        color: Colors.transparent,
        child: Padding(
            ...
            children: [
              Expanded(
                child: SongCardWidget(
                  showOptions: true,
                  onTap: () {
                    playerCubit.beatsMusicPlayer.skipToQueueItem(index);
                  },
                  onOptionsTap: () {
                    showMoreBottomSheet(
                      context,
