...
import 'package:beats_music/screens/screen/player_views/lyrics_search.dart';
...
class _FullscreenLyricsViewState extends State<FullscreenLyricsView> {
  ...
  void initState() {
    ...
    _currentTrackId = playerCubit.BeatsPlayer.currentTrackInfo.id;
  ...
  void _openSettingsMenu(LyricsState state, BeatsPlayerCubit playerCubit) {
    _hideControlsTimer?.cancel();
  ...
  Widget _buildBackground(BeatsPlayerCubit playerCubit) {
    return StreamBuilder<MediaItem?>(
      stream: playerCubit.BeatsPlayer.mediaItem,
      builder: (context, snapshot) {
        final currentTrack = playerCubit.BeatsPlayer.currentTrackInfo;
        final artworkUrl =
  ...
  Widget _buildTopBar(BeatsPlayerCubit playerCubit, bool isDesktop) {
    return Container(
            ...
            children: [
              ...
              BlocBuilder<LyricsCubit, LyricsState>(
                builder: (context, state) {
                  return IconButton(
                    onPressed: () =>
                        _openSettingsMenu(state, playerCubit),
  ...
  Widget _buildBottomControls(
      BeatsPlayerCubit playerCubit, bool isDesktop) {
    final l10n = AppLocalizations.of(context)!;
...
class _FullscreenSyncedLyricsState extends State<FullscreenSyncedLyrics> {
  ...
  void _forceSyncRecalculation() {
    final player = context.read<BeatsPlayerCubit>().BeatsPlayer.engine;
  ...
  void _setupPositionListener() {
    final playerCubit = context.read<BeatsPlayerCubit>();
    _positionSubscription = playerCubit
        .BeatsPlayer.engine.positionStream
        .listen((rawPosition) {
      if (!mounted) return;
