...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void initState() {
    ...
    _sourceEngine = SourceEngine.eng_YTV; // Default to YTVideo

    setState(() {
      availableSourceEngines().then((value) {
        availSourceEngines = value;
        // Use YTVideo if available, otherwise use first available engine
        _sourceEngine = value.contains(SourceEngine.eng_YTV)
            ? SourceEngine.eng_YTV
            : (value.isNotEmpty ? value[0] : SourceEngine.eng_YTV);
      });
    });
