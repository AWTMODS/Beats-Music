...
/// Fetches trending Malayalam songs from YouTube Music
/// Returns a list of formatted song items
Future<List<Map<String, dynamic>>> fetchMalayalamSongs() async {
  try {
    // YouTube Music search endpoint for Malayalam trending songs
    ...
    // Request body for Malayalam trending songs search
    ...
    if (response.statusCode != 200) {
      dev.log('Failed to fetch Malayalam songs: ${response.statusCode}',
          name: 'MalayalamSongs');
      return [];
    }
    ...
    if (contents == null || contents.isEmpty) {
      dev.log('No contents found in Malayalam songs response', name: 'MalayalamSongs');
      return [];
    }
    ...
    for (var section in contents) {
      ...
      for (var item in shelfContents) {
        try {
          ...
          dev.log('Error parsing Malayalam song item: $e', name: 'MalayalamSongs');
          continue;
        }
      }
    ...
    dev.log('Fetched ${songs.length} Malayalam songs', name: 'MalayalamSongs');
    return songs;
  } catch (e) {
