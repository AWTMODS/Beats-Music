...
/// Fetches trending Tamil songs from YouTube Music
/// Returns a list of formatted song items
Future<List<Map<String, dynamic>>> fetchTamilSongs() async {
  try {
    // YouTube Music search endpoint for Tamil trending songs
    ...
    // Request body for Tamil trending songs search
    ...
    if (response.statusCode != 200) {
      dev.log('Failed to fetch Tamil songs: ${response.statusCode}',
          name: 'TamilSongs');
      return [];
    }
    ...
    if (contents == null || contents.isEmpty) {
      dev.log('No contents found in Tamil songs response', name: 'TamilSongs');
      return [];
    }
    ...
    for (var section in contents) {
      ...
      for (var item in shelfContents) {
        try {
          ...
          dev.log('Error parsing Tamil song item: $e', name: 'TamilSongs');
          continue;
        }
      }
    ...
    dev.log('Fetched ${songs.length} Tamil songs', name: 'TamilSongs');
    return songs;
  } catch (e) {
