...
class ListeningStatisticsService {
  ...
  Future<void> recordPlay({
    ...
    try {
      ...
      await isar.writeTxn(() async {
        ...
        await isar.playHistoryDBs.put(PlayHistoryDB(
          songId: songId,
          songTitle: songTitle,
          artist: artist,
          genre: genre,
  ...
  Future<void> _updateSongStatistics(
    ...
    SongStatisticsDB? stats = await isar.songStatisticsDBs
        .filter()
    ...
    await isar.songStatisticsDBs.put(stats);
  }
  ...
  Future<void> _updateArtistStatistics(
    ...
    // Find existing statistics
    ArtistStatisticsDB? stats = await isar.artistStatisticsDBs
        .filter()
  ...
  Future<List<ArtistStatisticsDB>> getTopArtists({int limit = 20}) async {
    final isar = await BeatsMusicDBService.db;
    return await isar.artistStatisticsDBs
        .where()
        .sortByPlayCountDesc()
        .limit(limit)
        .findAll();
  ...
  Future<List<PlayHistoryDB>> getRecentlyPlayed({int limit = 50}) async {
    final isar = await BeatsMusicDBService.db;
    return await isar.playHistoryDBs
        .where()
        .sortByPlayedAtDesc()
        .limit(limit)
        .findAll();
  ...
  Future<int> getTotalListeningTime() async {
    final isar = await BeatsMusicDBService.db;
    final allStats = await isar.songStatisticsDBs.where().findAll();
  ...
  Future<Map<String, dynamic>> getTotalListeningStats() async {
    ...
    final allSongs = await isar.songStatisticsDBs.where().findAll();
  ...
  Future<void> clearAllStatistics() async {
    final isar = await BeatsMusicDBService.db;
    await isar.writeTxn(() async {
      await isar.playHistoryDBs.clear();
      await isar.songStatisticsDBs.clear();
      await isar.artistStatisticsDBs.clear();
    });
