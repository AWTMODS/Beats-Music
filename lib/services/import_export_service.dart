...
class ImportExportService {
  ...
  static Future<bool> isPlaylistExists(String playlistName) async {
    ...
    final _list = await BeatsMusicDBService.getPlaylists4Library();
  ...
  static Future<String?> exportPlaylist(String playlistName,
    ...
    final mediaPlaylistDB = await BeatsMusicDBService.getPlaylist(playlistName);
    if (mediaPlaylistDB != null) {
      try {
        // Fetch all media items in the playlist.
        List<MediaItemDB>? playlistItems =
            await BeatsMusicDBService.getPlaylistItems(mediaPlaylistDB);
        ...
        if (playlistItems != null) {
          ...
          final Map<String, dynamic> playlistMap = {
            '_meta': {
              'generated_by':
                  'BeatsMusic - Open Source Music Streaming Application',
              'version':
                  'v${packageInfo.version}+${int.parse(packageInfo.buildNumber) % 1000}',
              'exportedAt': DateTime.now().toIso8601String(),
              'note':
                  'This file is automatically generated by BeatsMusic and is intended solely for importing playlists within the application. Manual modification of this file is strongly discouraged, as it may result in errors, data inconsistency, or failed imports. For support or more information, please visit: https://github.com/HemantKArya/BeatsMusicTunes.',
            },
          ...
          final path = await writeToJSON(
              '${mediaPlaylistDB.playlistName}_BeatsMusicPlaylist.json',
              playlistMap,
              path: filePath);
  ...
  static Future<String?> exportMediaItem(MediaItemDB mediaItemDB) async {
    try {
      ...
      final packageInfo = await PackageInfo.fromPlatform();
      mediaItemMap['_meta'] = {
        'generated_by': 'BeatsMusic - Open Source Music Streaming Application',
        'version':
            'v${packageInfo.version}+${int.parse(packageInfo.buildNumber) % 1000}',
        'exportedAt': DateTime.now().toIso8601String(),
        'note':
      ...
      final path = await writeToJSON(
          '${mediaItemDB.title}_BeatsMusicSong.json', mediaItemMap);
      ...
      await readFromJSON(filePath).then((playlistMap) async {
        if (playlistMap != null && playlistMap.isNotEmpty) {
          ...
          for (final mediaItemMap in playlistMap['mediaItems']) {
            ...
            await BeatsMusicDBService.addMediaItem(mediaItemDB, playlistName);
    ...
    try {
      ...
      await readFromJSON(filePath).then((mediaItemMap) {
        if (mediaItemMap != null && mediaItemMap.isNotEmpty) {
          final mediaItemDB = MediaItemDB.fromMap(mediaItemMap);
          BeatsMusicDBService.addMediaItem(mediaItemDB, "Imported");
