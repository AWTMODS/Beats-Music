...
class PlayerErrorHandler {
  ...
  PlayerErrorType categorizeError(dynamic error) {
    // Check typed exceptions first (most specific)
    if (error is PluginNotLoadedException || error is PluginNotFoundException) {
      return PlayerErrorType.pluginError;
    }
    if (error is SocketException) return PlayerErrorType.networkDropped;
    if (error is TimeoutException) return PlayerErrorType.networkDropped;
    ...
    } else if (msg.contains('plugin is not loaded') ||
        msg.contains('plugin not loaded') ||
        msg.contains('plugin not found') ||
        msg.contains('pluginnotloaded') ||
        msg.contains('pluginnotfound')) {
      return PlayerErrorType.pluginError;
    } else if (msg.contains('malformed media id') ||
        msg.contains('cannot resolve stream') ||
  ...
  String _getUserFriendlyErrorMessage(PlayerErrorType type, String message) {
    switch (type) {
      ...
      case PlayerErrorType.pluginError:
        return 'Plugin unavailable. Check your plugins in Settings.';
      default:
