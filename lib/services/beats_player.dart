...
import 'package:beats_music/core/models/exported.dart' hide MediaItem;
import 'package:beats_music/core/models/media_playlist_model.dart';
import 'package:beats_music/core/constants/sentinel_values.dart';
import 'package:beats_music/core/constants/setting_keys.dart';
import 'package:beats_music/core/di/service_locator.dart';
import 'package:beats_music/plugins/utils/media_id.dart';
...
import 'package:beats_music/services/player/stream_quality_selector.dart';
import 'package:beats_music/services/plugin/plugin_service.dart';
import 'package:beats_music/services/meta_resolver/smart_track_replacement_service.dart';
...
class BeatsMusicPlayer extends BaseAudioHandler
  ...
  late ListeningStatisticsService _statsService;
  ...
  Future<void> _initAudioSession() async {
    try {
      ...
      log('Audio session configured', name: 'BeatsMusicPlayer');
    } catch (e) {
      _audioSessionConfigured = false;
      log('Failed to initialize audio session: $e', name: 'BeatsMusicPlayer');
    }
  }
  ...
  Future<bool> _activateAudioSession() async {
    try {
      ...
      log('Failed to activate audio session: $e', name: 'BeatsMusicPlayer');
      return false;
    }
  }
  ...
  Future<void> _deactivateAudioSession() async {
    try {
      ...
      log('Failed to deactivate audio session: $e', name: 'BeatsMusicPlayer');
    }
  }
    ...
    try {
      ...
      if (gainsJson != null) {
        try {
          ...
          log('Failed to restore EQ gains: $e', name: 'BeatsMusicPlayer');
        }
      }
      ...
      log('Failed to restore engine settings: $e', name: 'BeatsMusicPlayer');
    }
  }
  ...
  void _initModules() {
    ...
    _statsService = ListeningStatisticsService();
  ...
  void _initSubscriptions() {
    ...
    _errorSub = engine.errorStream.listen((error) {
      log('Engine error: $error', name: 'BeatsMusicPlayer');
      final track = _queueManager.currentTrack;
      if (track != null) {
  ...
  Future<void> _enqueuePlayTrack(Track track,
    ...
    final completer = CancelableCompleter<void>(
      onCancel: () =>
          log('Canceled: ${track.title}', name: 'BeatsMusicPlayer'),
    );
  ...
  Future<void> _doPlay(
    ...
    try {
      ...
      try {
        ...
        log('_updateCurrentTrack failed: $e', name: 'BeatsMusicPlayer');
      }
      ...
      log('Now playing: ${track.title}', name: 'BeatsMusicPlayer');
      done();
    } on TimeoutException catch (e) {
      isResolving.add(false);
      if (!alive()) return;
      log('Timeout loading ${track.title}: $e', name: 'BeatsMusicPlayer');
      ...
      log('Failed to play ${track.title}: $e',
          name: 'BeatsMusicPlayer', error: e, stackTrace: stackTrace);
  ...
  Future<(Track, ResolvedMediaSource)> _resolveTrackWithAutoReplacement(
    ...
    // 1. Detect malformed IDs early to skip the failing resolve attempt (avoid toast/delay).
    final isMalformed =
        tryParseMediaId(track.id) == null && !isLocalMediaId(track.id);

    if (isMalformed) {
      log('Malformed media ID detected early: ${track.id}',
          name: 'BeatsMusicPlayer');
      final replacement = await _tryAutoResolveUnavailableTrack(
        track,
        MalformedMediaIdException(rawId: track.id),
      );

      if (replacement == null) {
        throw MalformedMediaIdException(
          rawId: track.id,
          message: 'No replacement found for malformed media ID',
        );
      }

      final resolved = await _resolver.resolve(replacement);

      // AUTO-HEAL: Permanently update the playlist/library with the working ID.
      unawaited(_smartTrackReplacementService
          .applyReplacement(
        originalTrack: track,
        replacement: replacement,
      )
          .then((_) {
        log('Auto-healed malformed ID: ${track.id} -> ${replacement.id}',
            name: 'BeatsMusicPlayer');
      }).catchError((e) {
    ...
    try {
      ...
      // AUTO-HEAL: Update the playlist/library if the replacement was due to
      // missing/broken IDs, so future plays are direct.
      if (error is PluginNotLoadedException ||
          error is PluginNotFoundException ||
          isMalformed) {
        unawaited(_smartTrackReplacementService
            .applyReplacement(
          originalTrack: track,
  ...
  bool _isAutoResolvableError(Object error) {
    ...
    return message.contains('no streams returned') ||
        ...
        message.contains('plugin not loaded') ||
        message.contains('plugin not found') ||
        message.contains('malformed media id') ||
        message.contains('cannot resolve stream');
  }
  ...
  void _preResolveNextTrack() {
    ...
    _preResolveOp!.value.then((result) async {
      ...
      log('Pre-resolve failed: $e', name: 'BeatsMusicPlayer');
      _clearPreloadedMarker();
      unawaited(engine.clearPreload());
    });
  }
  ...
  void _onTrackCompleted() {
    ...
    Future.microtask(() async {
      try {
        ...
        log('Auto-next failed: $e', name: 'BeatsMusicPlayer');
      } finally {
        _isAdvancing = false;
      }
    });
  ...
  Future<void> _retryCurrentTrack() async {
    ...
    log('Retrying: ${track.title} at $pos', name: 'BeatsMusicPlayer');
    try {
      _errorHandler.clearError();
  ...
  Future<void> revive() async {
    if (!_isDisposed) return;
    log('Reviving BeatsMusicPlayer...', name: 'BeatsMusicPlayer');
  ...
  Future<void> loadPlaylist(Playlist playlist,
    ...
    try {
      ...
      log('loadPlaylist failed',
          name: 'BeatsMusicPlayer', error: e, stackTrace: stack);
      SnackbarService.showMessage(
  ...
  Future<void> _cleanup() async {
    ...
    try {
      ...
      log('Error disposing engine: $e', name: 'BeatsMusicPlayer');
    }
