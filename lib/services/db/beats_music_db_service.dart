...
class BeatsMusicDBService {
  static late Future<Isar> db;
  static late String appSuppDir;
  static late String appDocDir;
  static final BeatsMusicDBService _instance = BeatsMusicDBService._internal();
  ...
  BeatsMusicDBService get instance => _instance;

  factory BeatsMusicDBService({String? appSuppPath, String? appDocPath}) {
    if (appSuppPath != null) {
  ...
  static Future<String> getDbBackupFilePath() async {
    ...
    backupPath = p.join(backupPath, 'beatsBackup', 'beats_backup_db.json');
    // }
    return backupPath;
  }
      ...
      if (!await dbFile.exists()) {
        for (var element in bPaths) {
          ...
          if (await backUpFile.exists()) {
            ...
            log("DB Restored from $element", name: "BeatsMusicDBService");
            break;
          }
        }
      }
    } catch (e) {
      log("Failed to restore DB", error: e, name: "BeatsMusicDBService");
    }
  }
    ...
    if (Isar.instanceNames.isEmpty) {
      ...
      if (!await dbFile.exists()) {
        ...
        await checkAndRestoreDB(dbFile.path, [
          ...
          p.join(appDocDir, 'beats_backup_db.isar'),
          p.join(appSuppDir, 'beats_backup_db.isar'),
        ]);
      }
      ...
      if (!await dbFile.exists() &&
        ...
        log("DB Copied to $appSuppDir", name: "BeatsMusicDBService");
        _db.close();
      }
  ...
  static Future<String?> createBackUp() async {
    try {
      ...
      final jsonString = jsonEncode({
        "_meta": {
          'generated_by': 'Beats Music - Open Source Music Streaming Application',
          'version':
              'v${packageInfo.version}+${int.parse(packageInfo.buildNumber) % 1000}',
          "created_at": DateTime.now().toIso8601String(),
          "note":
              "This file is an automatically generated full backup of Beats Music. It includes playlists, search history, settings, and other app data. Manual modification is strongly discouraged and may cause data corruption. For help, visit: https://github.com/AWTMODS/Beats-Music."
        },
      ...
      log("Failed to create backup", error: e, name: "BeatsMusicDBService");
    }
    return null;
  }
  ...
  static Future<bool> backupExists() async {
    try {
      ...
      log("No backup exists", error: e, name: "BeatsMusicDBService");
    }
    return false;
  }
  ...
  static Future<Map<String, dynamic>> restoreDB(
    ...
    if (checkResults["isValid"] == true && backupFile.existsSync()) {
      ...
      log("DB restored successfully", name: "BeatsMusicDBService");
      return {"success": true};
    }
  ...
  static Future<void> resetDB() async {
    ...
    isarDB.writeTxn(() async {
      ...
      log("DB reset successfully", name: "BeatsMusicDBService");
    });
  }
