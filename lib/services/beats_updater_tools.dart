...
Future<Map<String, dynamic>> sourceforgeUpdate(
  ...
  const url = 'https://sourceforge.net/projects/awtmods/best_release.json';
  final userAgent = {
    'win':
...
Future<Map<String, dynamic>> customUpdate(
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      final release = data[platformKey];
      if (release == null) throw Exception('No release for $platformKey');

      final version = (release['version'] as String?) ?? '0.0.0';
      final build = (release['build']?.toString()) ?? '0';
      final downloadUrl = (release['url'] as String?) ?? '';
...
/// New public API: try Custom JSON, then GitHub, then SourceForge.
Future<Map<String, dynamic>> getAppUpdates() async {
  // Try Custom JSON first, then GitHub, then SourceForge.
  Map<String, dynamic> updates;
  try {
    updates = await customUpdate();
  } catch (e) {
    log('Custom check failed, trying GitHub: $e', name: 'UpdaterTools');
    try {
      updates = await githubUpdate();
    } catch (e2) {
      log('GitHub check failed, trying SourceForge: $e2', name: 'UpdaterTools');
      try {
        updates = await sourceforgeUpdate();
      } catch (e3) {
        log('SourceForge check failed: $e3', name: 'UpdaterTools');
