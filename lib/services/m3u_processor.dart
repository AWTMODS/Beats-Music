...
String convertJsonToM3U(Map<String, dynamic> jsonData) {
  try {
    ...
    final generatedBy = meta['generated_by']?.toString() ?? "Beats";
    ...
    // Custom Beats metadata (ignored by standard M3U players).
    buffer.writeln("#Beats-GENERATED_BY: $generatedBy");
    buffer.writeln("#Beats-VERSION: $version");
    buffer.writeln("#Beats-EXPORTEDAT: $exportedAt");
    buffer.writeln("#Beats-NOTE: $note");
    buffer.writeln();
...
Map<String, dynamic> parseM3UToJson(String m3uContent) {
  try {
    ...
    for (final line in lines) {
      ...
      if (line.startsWith('#Beats-GENERATED_BY:')) {
        meta['generated_by'] = line.split(':').last.trim();
      } else if (line.startsWith('#Beats-VERSION:')) {
        meta['version'] = line.split(':').last.trim();
      } else if (line.startsWith('#Beats-EXPORTEDAT:')) {
        meta['exportedAt'] = line.split(':').last.trim();
      } else if (line.startsWith('#Beats-NOTE:')) {
