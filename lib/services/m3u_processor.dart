...
String convertJsonToM3U(Map<String, dynamic> jsonData) {
  try {
    ...
    final generatedBy = meta['generated_by']?.toString() ?? "BeatsMusic";
    ...
    // Custom BeatsMusic metadata (ignored by standard M3U players).
    buffer.writeln("#BeatsMusic-GENERATED_BY: $generatedBy");
    buffer.writeln("#BeatsMusic-VERSION: $version");
    buffer.writeln("#BeatsMusic-EXPORTEDAT: $exportedAt");
    buffer.writeln("#BeatsMusic-NOTE: $note");
    buffer.writeln();
...
Map<String, dynamic> parseM3UToJson(String m3uContent) {
  try {
    ...
    for (final line in lines) {
      ...
      if (line.startsWith('#BeatsMusic-GENERATED_BY:')) {
        meta['generated_by'] = line.split(':').last.trim();
      } else if (line.startsWith('#BeatsMusic-VERSION:')) {
        meta['version'] = line.split(':').last.trim();
      } else if (line.startsWith('#BeatsMusic-EXPORTEDAT:')) {
        meta['exportedAt'] = line.split(':').last.trim();
      } else if (line.startsWith('#BeatsMusic-NOTE:')) {
